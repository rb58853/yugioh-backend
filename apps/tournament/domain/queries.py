...
class TournamentQueries(AbstractQueries):
    ...
    def create(self, name, manager, address, start_date=None, finish_date = None):
        start_date = datetime.date(2000, 1, 1) # FIXME: Quitar
        d = Tournament.objects.create(name=name, address=address, start_date= start_date, 
    ...
    def champions_archetypes(start_date, end_date):
        ...
        ).values('deck').annotate(count = Count('deck')).order_by('-count')
...
# def Consult10():
    # return Deck.objects.filter(id__in = Participant.objects.all().values('deck')).values('arquetipo').annotate(count = Count('arquetipo')).order_by('-count')